	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getPathInfo().toLowerCase().equals("/complete")) {
			...
			if (order.getOrder_status() != 1) {
				// Chưa xác thực kh thể hoàn thành
			...
			ArrayList<OrderDetail> orders = orderDAO.getAllOrderDetails(order_id);
			for (OrderDetail orderDetail : orders) {
				int result = productDAO.decreaseProductQuantity(orderDetail.getProduct_id(), orderDetail.getSize(),
						orderDetail.getQuantity());
				if (result == -2) {
					out.write(gson.toJson(new JsonResponse(false, "Sản phẩm tồn kho không đủ")));
					return;
				}
				if (result < 0) {
					out.write(gson.toJson(new JsonResponse(false, "Có lỗi xảy ra")));
		...
		if (request.getPathInfo().toLowerCase().equals("/cancel")) {
			...
			if (order.getOrder_status() != 0) {
				// Đơn hàng đã xác thực kh thể hủy
